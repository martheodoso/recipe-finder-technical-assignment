...
const Tag = ({ title, handleTagClick }: TagProps) => {
	return (
		<div className="bg-blue-800 text-teal-50 px-3 py-1 rounded-full text-sm font-semibold mr-2 mb-2 w-fit flex items-center">
			<span data-testid={`title-${title}`}>{title}</span>
			<button data-testid={title} type="submit" className="ml-1 text-gray-400 hover:text-gray-600 dark:hover:text-gray-300 focus:outline-none"
				formAction={`/api/remove-filters?removeTag=${title}`} onClick={handleTagClick}>
